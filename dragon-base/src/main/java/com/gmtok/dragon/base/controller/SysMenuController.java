...
import com.gmtok.dargon.base.service.SysMenuService;
...
public class SysMenuController {
    @Autowired
    private SysMenuService ${serviceVariableName};
 ...
 pid  String         
    ...
    @GetMapping("{id}")
    public ResponseEntity< SysMenu > get ( @PathVariable ${primaryKeyTypeString} id ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.selectById( id ));
    }

    @GetMapping
    public ResponseEntity< PageInfo<SysMenu> > list ( PagingRequest pagingRequest ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.listPage( pagingRequest ));
    }

    @PostMapping
    public ResponseEntity insert ( @RequestBody SysMenu ${entityVariableName} ){
    ...
    public ResponseEntity update ( @PathVariable ${primaryKeyTypeString} id,
                                              @RequestBody SysMenu ${entityVariableName} ){
