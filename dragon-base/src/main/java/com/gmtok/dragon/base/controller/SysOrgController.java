...
import com.gmtok.dargon.base.service.SysOrgService;
...
public class SysOrgController {
    @Autowired
    private SysOrgService ${serviceVariableName};
 ...
 orgPid  String         
    ...
    @GetMapping("{id}")
    public ResponseEntity< SysOrg > get ( @PathVariable ${primaryKeyTypeString} id ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.selectById( id ));
    }

    @GetMapping
    public ResponseEntity< PageInfo<SysOrg> > list ( PagingRequest pagingRequest ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.listPage( pagingRequest ));
    }

    @PostMapping
    public ResponseEntity insert ( @RequestBody SysOrg ${entityVariableName} ){
    ...
    public ResponseEntity update ( @PathVariable ${primaryKeyTypeString} id,
                                              @RequestBody SysOrg ${entityVariableName} ){
