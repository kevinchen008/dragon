...
import com.gmtok.dargon.base.service.SysMenuFunctionService;
 ...
 */
@RestController
@RequestMapping("sys-menu-functions")
public class SysMenuFunctionController {
    @Autowired
    private SysMenuFunctionService ${serviceVariableName};
 ...
 remark  String         
    ...
    @GetMapping("{id}")
    public ResponseEntity< SysMenuFunction > get ( @PathVariable ${primaryKeyTypeString} id ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.selectById( id ));
    }

    @GetMapping
    public ResponseEntity< PageInfo<SysMenuFunction> > list ( PagingRequest pagingRequest ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.listPage( pagingRequest ));
    }

    @PostMapping
    public ResponseEntity insert ( @RequestBody SysMenuFunction ${entityVariableName} ){
    ...
    public ResponseEntity update ( @PathVariable ${primaryKeyTypeString} id,
                                              @RequestBody SysMenuFunction ${entityVariableName} ){
