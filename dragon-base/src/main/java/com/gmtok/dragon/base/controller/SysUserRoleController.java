...
import com.gmtok.dargon.base.service.SysUserRoleService;
...
public class SysUserRoleController {
    @Autowired
    private SysUserRoleService ${serviceVariableName};
 ...
 id  String         
    ...
    @GetMapping("{id}")
    public ResponseEntity< SysUserRole > get ( @PathVariable ${primaryKeyTypeString} id ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.selectById( id ));
    }

    @GetMapping
    public ResponseEntity< PageInfo<SysUserRole> > list ( PagingRequest pagingRequest ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.listPage( pagingRequest ));
    }

    @PostMapping
    public ResponseEntity insert ( @RequestBody SysUserRole ${entityVariableName} ){
    ...
    public ResponseEntity update ( @PathVariable ${primaryKeyTypeString} id,
                                              @RequestBody SysUserRole ${entityVariableName} ){
