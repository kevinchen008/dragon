...
import com.gmtok.dargon.base.service.SysRoleMenuFunctionService;
 ...
 */
@RestController
@RequestMapping("sys-role-menu-functions")
public class SysRoleMenuFunctionController {
    @Autowired
    private SysRoleMenuFunctionService ${serviceVariableName};
 ...
 menuFunctionId  String         
    ...
    @GetMapping("{id}")
    public ResponseEntity< SysRoleMenuFunction > get ( @PathVariable ${primaryKeyTypeString} id ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.selectById( id ));
    }

    @GetMapping
    public ResponseEntity< PageInfo<SysRoleMenuFunction> > list ( PagingRequest pagingRequest ){
        return ResponseEntity.ok().setResponseContent(${serviceVariableName}.listPage( pagingRequest ));
    }

    @PostMapping
    public ResponseEntity insert ( @RequestBody SysRoleMenuFunction ${entityVariableName} ){
    ...
    public ResponseEntity update ( @PathVariable ${primaryKeyTypeString} id,
                                              @RequestBody SysRoleMenuFunction ${entityVariableName} ){
